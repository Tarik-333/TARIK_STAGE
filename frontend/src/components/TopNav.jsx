...
const TopNav = () => {
    ...
    const navItems = user?.role === 'admin' 
        ? [...baseNavItems.slice(0, 3), { path: '/users', label: 'Équipe' }, ...baseNavItems.slice(3)]
        : baseNavItems;
    ...
    return (
        <header className="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 h-16 shrink-0 sticky top-0 z-50 transition-colors">
            <div className="max-w-[1600px] w-full mx-auto h-full px-4 md:px-10 flex items-center justify-between">
                {/* Logo area */}
                <div className="flex items-center space-x-6">
                    <Link to="/dashboard" className="flex items-center gap-2">
                        <Logo className="h-8 w-auto" />
                        <span className="text-lg font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">ValaFlow</span>
                    </Link>
                        ...
                        {showNotifications && (
                            <div className="absolute right-0 mt-3 w-80 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden z-50">
                                <div className="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gray-50/50 dark:bg-gray-800/50">
                                    ...
                                    {unreadCount > 0 && (
                                        <button onClick={markAllAsRead} className="text-xs font-medium text-vf hover:underline dark:text-vf/90">
                                            Tout marquer lu
                                        </button>
                                    )}
                                </div>
                                ...
                                <div className="max-h-80 overflow-y-auto custom-scrollbar">
                                    {notifications.length === 0 ? (
                                        ...
                                        notifications.map(notif => (
                                            <div 
                                                key={notif.id} 
                                                className={`p-4 border-b border-gray-50 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors flex items-start space-x-3 cursor-pointer ${notif.is_read ? 'opacity-70' : 'bg-vf-soft/50 dark:bg-vf/10'}`}
                                            ...
                                            >
                                                ...
                                                {!notif.is_read && (
                                                    <div className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-vf"></div>
                                                )}
                                            </div>
                                        ))
                                    )}
